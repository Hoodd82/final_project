...
model_2_dt <- flights_dt %>%
  ...
  select(-c(arr_delay, engines, seats, lat, lon, wind_dir, year, temp, precip, minute, distance, visib, flight))
...
```{r}
model_3 <- lm(dep_delay ~ air_time + aircraft_age + alt + hour + dewp + wind_speed
              + pressure + dewp:humid + pressure:dewp, data = model_2_dt)
summary(model_3)
```
...
flights_decision_tree <- flights_dt %>%
  filter(origin == "EWR") %>% 
  select(dep_delay, air_time, distance, engines, engine, 
         aircraft_age, lat, lon, alt, hour, temp, precip, humid, visib, pressure,
         dewp, wind_dir, wind_speed) %>% 
  mutate(engines = as.factor(engines),
         late_departure = ifelse(dep_delay > 0, "Late", "Ontime/Early")) %>% 
  select(-dep_delay)
