...
flights_decision_tree <- flights_dt %>%
  filter(origin == "EWR") %>% 
  select(sched_dep_time, dep_delay, distance, engines, engine, 
  ...
  mutate(engines = factor(engines),
         ...
         month = month(sched_dep_time, label = TRUE),
         late_departure = factor(ifelse(dep_delay > 0, "Late", "Ontime/Early"))) %>%
  ...
  select(-c(dep_delay, sched_dep_time)) %>% 
  na.omit()
